...
public class DigiOSVoiceUIInterface : MonoBehaviour
{
    ...
    //Voice UI listeners
    AndroidJavaObject voiceUI_Listener_restart_spaces = null;
    AndroidJavaObject voiceUI_Listener_recenter = null;

    //Voice Commands
    string command1 = "restart spaces";
    string command2 = "recenter";
    ...
    void RegisterVoiceCommands()
    {
        ...
        voiceUI_Listener_restart_spaces = new AndroidJavaObject("com.digilens.digios_unity_plugin.utils.VoiceUI_Listener",
             command1, voiceUI_Interface_Constants.GetStatic<int>("Voice_Command_CONFIG_TYPE_FEEDBACK_ONLY"), gameObject.name,
             "RestartSpaces_Callback");
        voiceUI_Listener_recenter = new AndroidJavaObject("com.digilens.digios_unity_plugin.utils.VoiceUI_Listener",
            command2, voiceUI_Interface_Constants.GetStatic<int>("Voice_Command_CONFIG_TYPE_FEEDBACK_ONLY"), gameObject.name,
            "Recenter_Callback");


        //Adding listeners to the appropriate language VoiceUI_Model. 
        voiceUI_Model.Call("addVoiceUI_Listener", voiceUI_Listener_restart_spaces);
        voiceUI_Model.Call("addVoiceUI_Listener", voiceUI_Listener_recenter);
    ...
    public bool VoiceUI_Stop()
    {
        if (voiceUI_Interface != null) //If voice UI interface is initalized, disconnect service
        {
            ...
            voiceUI_Listener_restart_spaces.Dispose();
            voiceUI_Listener_recenter.Dispose();
    ...
    public void VoiceUI_Callback1(string voice_command)
    {
        ...
        Debug.Log("resetting spaces service...");
    ...
    /// <summary>
    /// Terminates the snapdragon spaces background service
    /// </summary>
    void KillSpacesProcess()
    {
        using (AndroidJavaClass unityClass = new AndroidJavaClass("com.unity3d.player.UnityPlayer"))
        {
            using (AndroidJavaObject activity = unityClass.GetStatic<AndroidJavaObject>("currentActivity"))
            {
                using (var plugin = new AndroidJavaObject("com.digilens.digi_os_sdk.DigiOS_SDK"))
                {
                    Debug.Log("Restarting spaces service");
                    plugin.Call("restartSpacesServices", activity.Call<AndroidJavaObject>("getApplicationContext"));
                }
            }
        }
    }
